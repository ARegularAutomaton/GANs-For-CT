...
for epoch in range(num_epochs):
    ...
    for i, fake_batch in enumerate(X_dataloader):
        ...
        # fakeB = netGA(realA)
        # ssim_loss: torch.Tensor = piq.SSIMLoss(data_range=1.)(fakeB, realB)
        # conformity_loss = 1e-3 * ssim_loss
        ...
        x1 = netGA(physics.pseudo(meas0))
        loss_domain = mse(x1, fakeB.detach())
        # loss_c1 = 0 * mse(x1, fakeB_cycle)

        #-------------------------
        # eq loss
        x2 = transform.apply(x1.detach())
        meas2 = physics.forw(x2)
        x3 = netGA(physics.pseudo(meas2))
        loss_eq = mse(x3, x2)

        loss = loss_domain + loss_eq
        loss.backward()
        ...
        if epoch > 0 and epoch % 10 == 0:
                    ...
                    os.makedirs(save_path, exist_ok=True)
                    torch.save(state,
                               os.path.join(save_path, 'ckp_{}_ExtendedCycle.pth.tar'.format(now)))
