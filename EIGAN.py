...
def get_random_batch(dataloader):
    ...
    # Generate a random index within the range of number of batches
    ...
    for batch_index, batch_data in enumerate(dataloader):
        if batch_index == random_index:
            random_batch = batch_data[0].to(device)
            break
    return random_batch
...
for epoch in range(num_epochs):
    ...
    for i, data in enumerate(dataloader):
        ...
        if epoch > 0 and epoch % 10 == 0:
                    ...
                    os.makedirs(save_path, exist_ok=True)
                    torch.save(state,
                               os.path.join(save_path, 'ckp_{}_EIGAN.pth.tar'.format(now)))
